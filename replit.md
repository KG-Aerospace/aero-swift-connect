...
- **Parts Pricing Integration**: Real-time pricing data integrated from uploaded price lists. Database stores hundreds of aviation parts with pricing from $0.18 to $170,000+ USD. Pricing information automatically displays in Orders page and Draft Orders part selection. Approximate prices show as "~$X.XX USD" throughout the interface. Draft orders now include unitPrice field populated automatically from parts database when creating or updating items.
- **Customer Requests UI Enhancement**: Fully redesigned as ergonomic table interface with inline part details. Shows Part Number, description, quantity, AC type (with proper object-to-string conversion), and estimated price for all items directly in collapsed view. Features advanced search across all fields (CR numbers, subjects, parts), sortable columns, icon-based actions with tooltips, expandable rows for detailed views, and status filtering. Displays up to 3 line items in collapsed state with "... and X more items" indicator for longer lists.
- **Enhanced Draft Order Fields**: Added alternates and unitPrice fields to draft orders schema. AC Type and Engine Type display properly handles both string and object formats. Pricing information is automatically populated from parts database on draft order creation and updates.
